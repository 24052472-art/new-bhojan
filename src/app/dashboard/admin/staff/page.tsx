...
export default function StaffManagement() {
  ...
  async function fetchStaff(restaurantId: string) {
    setIsLoading(true);
    try {
      const { data } = await supabase.from("profiles").select("*").eq("restaurant_id", restaurantId).neq("role", "owner").neq("role", "super_admin");
      setStaff(data || []);
    } finally {
      setIsLoading(false);
    }
                 ...
                 <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                       ...
                       <select 
                         ...
                         className="w-full h-14 bg-slate-50 border border-slate-100 rounded-2xl px-6 text-sm font-bold outline-none focus:border-[#ff5a2c] transition-all appearance-none"
                       >
